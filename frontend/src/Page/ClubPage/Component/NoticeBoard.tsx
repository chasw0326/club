...
const PostList = ({setPostInfo, setCategory} : {setPostInfo : any, setCategory : any}) => {
  const clubID = window.location.pathname.split('/')[2];
  ...
  const postClick = (e : SyntheticEvent) => {
    ...
    setPostInfo({title : target.dataset.title, content : target.dataset.content, postId : target.dataset.postid});
    setCategory('post');
  }
...
const Post = ({postInfo}: {postInfo : any}) => {
  ...
  const fetchData = async () => {
    const fData = await fetch(`${process.env.REACT_APP_TEST_API}/api/post`);
      const [status, res] = await getAPI(`/api/comment?postId=${postInfo.postId}`);
  ...
  return (
          <div className="ClubPage-postBox">
            <div className="ClubPage__text--title">{postInfo?.title}</div>
